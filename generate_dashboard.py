...
def build_html(data, targets, alerts, notes, report_date=None, report_time=None):
    ...
    # Build HTML in sections — avoids large f-string (Python 3.14 compatible)
    _parts = []
    def _a(s): _parts.append(s)
...
""")
    _a(f"""      <div style="font-size:20px;font-weight:900;letter-spacing:.3px;line-height:1.2">CRM NARROW COMPLEX — DAILY OPERATIONS DASHBOARD</div>
    </div>
  </div>
  <div style="display:flex;align-items:center;gap:8px">
    ...
    <div style="background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);
""")
    _a(f"""                border-radius:8px;padding:8px 18px;text-align:center">
...
<div style="display:flex;gap:8px;margin-bottom:8px">
  <!-- ANNEALING -->
""")
    _a(f"""  <div class="card" style="flex:1">
...
""")
    _a(f"""          <span style="margin-left:12px">Old charges: <b style="color:{TXT}">{int(ann.get('chg_old',0))}</b></span>
...
""")
    _a(f"""      {_fin_row("CRCA Slitting",  crs.get('crca_slitting', dict()).get('yesterday',0), unit="MT")}
    </div>
...
""")
    _a(f"""        <div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.7px;color:{MUT};margin-bottom:7px">Cumulative (MTD)</div>
...
""")
    _a(f"""        <div style="flex:{crs.get('slitting', dict()).get('tube',1)};background:{G};display:flex;align-items:center;justify-content:center">
...
""")
  ...
  <div style="width:330px;flex-shrink:0;display:flex;flex-direction:column;gap:10px">
    ...
    _a(f"""
""")
    _a('\n# =============================================================================\n#  SINGLE-MILL MIS PARSER (for separate CRM04 / CRM06 files)\n# =============================================================================\n\ndef parse_single_mill_mis(text, mill_name, report_day=None, month_start_row=None):\n    """\n    Parse a MIS file containing only ONE mill.\n    Mill label is provided externally (from filename).\n    Row numbering is continuous (e.g. 32-62 for a month).\n    month_start_row: continuous row number that equals day 1 of the month.\n    """\n')

    return ''.join(_parts)

def parse_single_mill_mis(text, mill_name, report_day=None,
                           month_start_row=None, col_map=None):
    """Parse a MIS file containing only ONE mill. Label from filename.
    col_map overrides MIS_COL column indices (from user Column Mapper)."""
    # Merge user mapping on top of defaults
    cm = dict(MIS_COL)
    if col_map:
        cm.update({k: v for k, v in col_map.items() if v is not None})
    ...
    if not nonempty:
        return {}
    chosen = None
    if report_day is not None and month_start_row:
        target_row = month_start_row + report_day - 1
        for r in nonempty:
            if r[0].strip().isdigit() and int(r[0].strip()) == target_row:
                chosen = r; break
    if chosen is None:
        chosen = nonempty[-1]
    g = lambda k: chosen[cm[k]] if cm.get(k) is not None and cm[k] < len(chosen) else ""
    roll, rr, skp = n(g("roll")), n(g("rr")), n(g("skp"))
    return {
        mill_name: {
            "day_total":  round(roll + rr + skp, 3),
...
def mill_name_from_filename(filename):
    """Extract CRM04 / CRM06 from filename like 'CRM04 MIS.xlsx'."""
    import re as _re
    ...
    if m:
        return f"CRM{m.group(2).zfill(2)}"
    return "CRM"


# =============================================================================
#  FLEXIBLE PARSERS — use user-supplied column mapping instead of hardcoded
# =============================================================================

def flexible_parse_mis(text, col_map, report_day=None):
    """Like parse_mis but uses col_map dict (key -> 0-based column index).
    col_map should have same keys as MIS_COL. Missing keys fall back to MIS_COL."""
    merged = dict(MIS_COL)
    merged.update({k: v for k, v in col_map.items() if v is not None})
    ...
    def flush(label, rows):
        ...
        if report_day is not None:
            for r in rows:
                d = r[0].strip()
                if d.isdigit() and int(d) == report_day and len(r) > 6:
                    if any(c.strip() for c in r[1:7]):
                        chosen = r; break
        if chosen is None and nonempty:
            chosen = nonempty[-1]
        if chosen is None:
            return
        g = lambda k: chosen[merged[k]] if merged.get(k) is not None and merged[k] < len(chosen) else ""
        roll, rr, skp = n(g("roll")), n(g("rr")), n(g("skp"))
        rolling[label] = {
            "day_total":  round(roll + rr + skp, 3),
            "day_roll":   roll, "day_rr": rr, "day_skp": skp,
    ...
    for ln in lines:
        ...
        m = re.search(r"\b(CRM\d{2})\b", ln)
        if m:
            pending_label[0] = m.group(1)
            flush(pending_label[0], current_rows)
            current_rows = []
    if current_rows and pending_label[0]:
        flush(pending_label[0], current_rows)
    return rolling


def flexible_parse_anneal(text, col_map, report_day=None):
    """Like parse_anneal_mis but uses col_map dict."""
    merged = dict(ANN_COL)
    merged.update({k: v for k, v in col_map.items() if v is not None})
...
def get_sheet_headers(filepath_or_bytes, sheet_name=None):
    """Return (headers_list, preview_df, sheet_names) for the mapper UI.
    headers_list: list of 'Col A', 'Col B'... with first non-empty value shown.
    preview_df: first 5 data rows as list of lists.
    """
    import io, openpyxl
    if isinstance(filepath_or_bytes, (bytes, bytearray)):
        wb = openpyxl.load_workbook(io.BytesIO(filepath_or_bytes),
    ...
    # Build header labels: "Col A (val1 / val2 ...)" using first non-empty values
    headers = []
    for ci in range(max_cols):
        ...
        for row in all_rows:
            ...
            if len(samples) >= 3:
                break
        label = f"Col {col_letter}  [{' / '.join(samples[:2])}]" if samples else f"Col {col_letter}"
        headers.append((ci, label))

    return headers, all_rows[:8], names
